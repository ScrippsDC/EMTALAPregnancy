...
```{r}
emtala_psych_crit_short <- events_all %>%
  filter(grepl('2400|2401|2402|2403|2404|2405|2406|2407|2408|2409|2410|2411', deficiency_tag))

emtala_psych_crit_short$year <- format(as.Date(emtala_psych_crit_short$inspection_date, format="%Y/%M/%D"),"%Y")
```
...
##Reading in violations from 2011-Q3 2022 post manual review of inspection_text to determine if violation involved a pregnant patient: 
```{r}
...
##“Even before the fall of Roe, 385 hospitals violated EMTALA while attending to pregnant patients.” (386 hospitals since 2011 but one hospital had a violation after the fall of Roe: 9/2/2022 TAYLORVILLE MEMORIAL HOSPITAL)

```{r}
n_distinct(emtala_pregnant$facility_id)

#filtering for any violations after the supreme court overturning Roe v. Wade: 

emtala_pregnant%>%
 dplyr::filter(inspection_date > '2022-06-24')
```

## TEXT: “Emergency departments have been violating EMTALA statutes relating to pregnant people for over 10 years (at least since 2011).” 
...
##"“After the fall of Roe v. Wade, the Biden administration has continued to remind emergency departments of their duty to serve pregnant patients by reinforcing EMTALA, the anti-patient dumping law. Of the 358 investigations that found EMTALA investigations relating to pregnant patients since 2013, only 11 (3%) led to a monetary fine.” 
...
n_distinct(emtala_psych_crit_short$EVENT_ID)

#number of EMTALA pregnant patient investigations since 2013
emtala_pregnant_post_2013 <- emtala_pregnant %>%
filter(inspection_date > '2012-12-31')

n_distinct(emtala_pregnant_post_2013$EVENT_ID)
  
#percent of EMTALA investigations involving a pregnant patient since 2011 
413/2694 

#percent of EMTALA investigations involving a pregnant patient resulting in a monetary fine (civil monetary penalty from HHS OIG office)
11/347
```

##“Out of all EMTALA violations, when it comes to pregnant patients, emergency departments are failing to provide medical screening examinations the most.”  
...
## “When it comes to EMTALA, hundreds of emergency departments have violated the rights of pregnant women but only 11 ERs have had monetary penalties.” 
## ALT:"CMS only imposed monetary penalties on 11 ERs in the past 10 years (2013-2022) for EMTALA violations relating to pregnant people -- even though it found hundreds (339) of facilities committing these violations."

```{r}
n_distinct(emtala_pregnant_post_2013$facility_id)
n_distinct(emtala_pregnant_post_2013$EVENT_ID)
```

Lets make a visualization!
```{r}
emtala_preg_def_count <- emtala_pregnant %>%
count(deficiency_tag)
...
```{r}
ggplot(data=emtala_preg_def_count, aes(x=deficiency_tag, y=n))+
  geom_bar(stat = "identity")+
  geom_text(aes(label=deficiency_tag), vjust=-0.3, size=2.5)+
  theme_minimal()
  
```

```{r}
#compare emtala preg serious violations rate by year to rate of emtala serious violations per year 

#serious violations - 2406-2409

#serious violations in 2011/all violations in 2011

emtala_psych_crit_short_def_count <- emtala_psych_crit_short%>%
  filter(deficiency_tag == "2406")%>%
  group_by(deficiency_tag)%>%
  count(year)

emtala_psych_crit_short_def_count$emtala_rate2406 <- 
  (emtala_psych_crit_short_def_count$n/6692)*100
...
emtala_preg_def_count$emtalapreg_rate2406 <- 
  (emtala_preg_def_count$n/693)*100

emtala_2406_list <- list(emtala_psych_crit_short_def_count, emtala_preg_def_count)

df <- Reduce(function(x, y) merge(x, y, all = TRUE), emtala_2406_list)

write_csv(df, "data/source/df.csv")

df_transpose <- read_csv("data/source/df_transpose.csv")
...
theme_minimal() +
  ...
  labs(title="EMTALA lack of medical screening",
       x = "Year",
       y = "Rate")
...
Which hospitals are rural/urban:
...
```{r}
turnaway <- read_xlsx("data/turnaway.xlsx")
