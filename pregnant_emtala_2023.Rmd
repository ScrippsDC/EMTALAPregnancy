...
# Groupby EVENT_ID, list violations by event, multiple rows per event even though defcncy_desc has been collapsed (same for all n rows)
...
# Only want one row per EVENT_ID
...
```{r}
# Doing the same thing but on the hospital level
...
# Doing the same thing but making counts instead of a list
...
```{r}
# Also for making the flourish chart
